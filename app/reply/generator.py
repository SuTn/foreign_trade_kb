...
def generate_reply(pipeline: RagPipeline, customer_id: str, chat_id: str,
                   incoming_message: str, style: str = "default") -> dict:
    """返回 {reply, sources}。不发送。style 决定候选表达风格。"""
    style_instruction = REPLY_STYLE_VARIANTS.get(style, "")
    system = REPLY_SYSTEM.format(style=style_instruction)
...
def regenerate_reply(pipeline: RagPipeline, customer_id: str, chat_id: str,
                     incoming_message: str, previous_style: str = "default") -> dict:
    """重新生成获得不同候选 (切换表达风格 + LLM 温度自然差异)。"""
    next_style = {"default": "concise", "concise": "warm", "warm": "formal",
                  "formal": "default"}.get(previous_style, "default")
    return generate_reply(pipeline, customer_id, chat_id, incoming_message, style=next_style)

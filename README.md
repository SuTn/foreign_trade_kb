...
2. 首页为仪表盘, 概览采集器状态、客户与知识库统计、近期活跃会话; `/api/collector/status` 返回 JSON 状态。
3. `/customers` 查看客户列表与画像; 客户列表以卡片网格展示, 含 WhatsApp 自动抓取的头像 (无头像时为首字母占位), 支持按名称/电话/公司/国家/画像字段实时搜索与筛选; 点击客户进入聊天视图。
4. 在客户聊天页面对最新消息请求辅助回复, 系统通过 RAG 召回画像/历史/产品知识后生成建议回复 (仅生成, 不自动发送, 业务员复制后手动发送)。
5. 客户详情页可一键生成「对话摘要」, 结构化展示意向车型/预算/目标国家/核心顾虑/待跟进事项, 便于快速复盘。

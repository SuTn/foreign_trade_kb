...
- [x] 2.3 新增 `POST /api/cleanup`（mode: chat|days，前端确认后调用）
- [x] 2.4 管理入口：模板页/按钮触发清理
- [x] 2.5 单测：按会话/按天数删除、画像与知识库保留断言
...
- [x] 4.1 全量 `pytest -q` 通过（新增 + 既有）
- [x] 4.2 `compileall -q app` 通过
- [x] 4.3 代码走读：清理保留画像/知识库、搜索各源正确、横幅轮询无泄漏

...
- [x] 1.3 `app/__main__.py` supervisor 循环：采集器进程异常退出自动拉起（正常退出/用户中断不重启）
- [x] 1.4 采集器 `__main__.py` 异常时以非 0 exit code 退出，供 supervisor 判定
...
- [x] 9.1 全量 `pytest -q` 通过（新增 + 既有）
- [x] 9.2 `compileall -q app` 通过
- [x] 9.3 代码走读确认无遗留每请求 store 新建与死代码

...
- [x] 1.3 配置 Playwright 与独立 Chrome user-data-dir，验证可打开 WhatsApp Web
...
- [x] 2.3 接入 Chroma 向量库（消息按 chatId+day 分组、知识 chunk 向量，幂等 upsert）
- [x] 2.4 实现 LLM 抽象层（默认云端 Claude/OpenAI，可切本地）
...
- [x] 5.1 设计可插拔插件接口与管线骨架（事件链式调用）
- [x] 5.2 实现多路召回（客户画像 + 历史聊天向量 + 产品知识向量 + BM25）
- [x] 5.3 实现 rerank（交叉编码器/云端 rerank）
- [x] 5.4 实现上下文压缩/去重与父子块展开
- [x] 5.5 预留查询理解/改写插件扩展点（MVP 可空实现）

## 6. 客户画像与分析

- [x] 6.1 实现客户实体匹配（手机号 + 显示名启发式 + 人工确认合并）
- [x] 6.2 实现 LLM 画像抽取/更新（从聊天摘要抽取字段，带时间戳与来源）
- [x] 6.3 实现画像手动编辑（人工值优先，不被自动抽取覆盖）
- [x] 6.4 实现客户分析生成（兴趣点/活跃度/跟进建议）

## 7. 辅助回复

- [x] 7.1 实现回复生成（RAG 召回画像+历史+产品知识 + 当前消息 → 建议回复）
- [x] 7.2 实现检索来源可追溯（展示支撑回复的片段）
- [x] 7.3 实现多候选回复（重新生成获得不同候选）
- [x] 7.4 确保仅生成不自动发送

## 8. Web 应用

- [x] 8.1 搭建 FastAPI + Jinja2/HTMX 骨架与本地 127.0.0.1 启动
...
- [x] 9.1 端到端联调：WhatsApp 同步 → 画像 → 回复生成 全链路
- [x] 9.2 端到端联调：本地知识导入 → RAG 检索 → 回复引用产品知识
- [x] 9.3 端到端联调：本地知识导入 → Wiki 页面生成 → 导出 Obsidian vault → 图谱查看
- [x] 9.4 验证幂等与去重（重复采集不产生重复记录）
- [x] 9.5 验证只读约束（全程不发送任何 WhatsApp 消息）
- [x] 9.6 文档化封号风险提示与使用说明
